...
import {
GET_BY_EMPLOYEE, GET_EMPS_BY_PNAME
} from "../types";
...
export const getEmpByCodePrj = (name) => async (dispatch) => {
  // dispatch({ type: SET_LOADING, payload: true })

  await axios
    .get(projectsApi + `/api/report/view/${name}`)
    .then((res) => {
      // console.log(res)
      const emps = res.data.map((item) => ({
        ...item,
      ...
      // dispatch({ type: SET_LOADING, payload: false })
      dispatch({ type: GET_EMPS_BY_PNAME, payload: emps });
    })
    .catch((err) => console.log("Get project api error", err));
};